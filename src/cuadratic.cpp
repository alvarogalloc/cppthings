...
int main(int argc, const char **argv) {
  ...
  ecuation +=
      (b < 0) ? (std::to_string(b) + "x") : ("+" + std::to_string(b) + "x");
  ...
  if (c < 0) {
    c = abs(c);
    ecuation = "x = -(" + std::to_string(b) + ")" + "±sqrt(" +
               std::to_string(b) + "^2+4*" + std::to_string(a) + "*" +
  ...
  /* float result =  */
  ecuation += std::to_string(b) + "±sqrt(" +
              std::to_string((const int)pow(b, 2)) + "+" +
              std::to_string(4 * a * c) + ")/" + std::to_string(2 * a);
  std::cout << "despeje 2" << ecuation << "\n";
  ecuation = std::to_string(b) + "¬±sqrt(" +
             std::to_string((const int)pow(b, 2) + 4 * a * c) + ")/" +
             std::to_string(2 * a);
